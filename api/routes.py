...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if decision.mode == SearchMode.SQL and decision.sql_query_key:
            sql_results = await execute_sql_query(decision.sql_query_key, decision.sql_params or {})
...
async def index_document(request: IndexRequest):
    ...
    full_path = os.path.join(settings.PDF_STORAGE_PATH, request.chemin_fichier.lstrip("/"))

    if not os.path.exists(full_path):
        raise HTTPException(status_code=404, detail=f"Fichier non trouvé: {request.chemin_fichier}")
    ...
    try:
        ...
        if not text or len(text.strip()) < 20:
            return IndexResponse(success=False, document_id=request.document_id, n_chunks=0, message="Texte insuffisant")
        ...
        return IndexResponse(success=True, document_id=request.document_id, n_chunks=n_chunks, message=f"Indexé avec succès ({n_chunks} chunks)")
